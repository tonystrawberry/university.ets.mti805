...
int training_main()
{
	...
	for (int k = 0; k < NUM_BLUE; k++) {
		...
		for (int i = 0; i < img_mat.rows; i++) {
			for (int j = 0; j < img_mat.cols; j++) {
				Scalar intensity = img_mat.at<uchar>(i, j);
				training_mat.at<int>(k, ii) = intensity[0];
				ii++;
			}
		}
	}
	...
	double compactness = kmeans(training_mat, NB_CLUSTERS, labels,
		TermCriteria(TermCriteria::EPS + TermCriteria::COUNT, 10, 1.0),
		3, KMEANS_PP_CENTERS, centers);

	for (int k = 0; k < labels.rows; k++) {
		
		if (labels.at<int>(k, 0) == 1) {
			cout << "labels " << (k + 1) << " : " << labels.at<int>(k, 0) << endl;
		} else if (labels.at<int>(k, 0) == 2) {
			cout << "labels " << (k + 1) << " : " << labels.at<int>(k, 0) << endl;
		}
		else if (labels.at<int>(k, 0) == 3) {
			cout << "labels " << (k + 1) << " : " << labels.at<int>(k, 0) << endl;
		}
		else if (labels.at<int>(k, 0) == 4) {
			cout << "labels " << (k + 1) << " : " << labels.at<int>(k, 0) << endl;
		}
	}
	waitKey(0);
